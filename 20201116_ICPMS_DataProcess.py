...
# def import_single_header_dfs(names):
#     """
#     For importing excel sheets without multi-level column headers.
#     """
#     # Create an empty dataframe.
#     master = pd.DataFrame()
#     master2 = pd.DataFrame()
    
#     # read_excel will unmerge cells and assigned empty ones "Unnamed".
#     for workbook in file_names:
#         df = pd.read_excel(workbook)
#         master.append(df)
#         master2.add(df)
        
        
# def flatten_col_headers(df):
#     """
#     For importing excel sheets without multi-level column headers, this -
#     function will "flatten" two column headers into one based  on "Unnamed" 
#     being assigned to empty cells.
#     """
#     lst1 = [val for val in df.columns if "Unnamed" in val]
#     lst2 = []
#     for val, val2 in list(zip(df.columns, df.iloc[0])):
#         if "Unnamed" in val:
#             lst2.append(val2)
#     dct = dict(zip(lst1, lst2))
#     df_single_header = df.rename(dct, axis=1)
    
#     return df_single_header


# def single_header_df_remove_ISTD(df):
#     """
#     This works to remove ISTD columns if you don't have multi-level column 
#     headers.
#     """
#     for col in df.columns:
#         if "( ISTD )" in col:
#             df.drop(col, axis=1, inplace=True)
#     # Verify no columns with ISTD left.
#     for col in df.columns:
#         if "ISTD" in col:
#             print(col)
